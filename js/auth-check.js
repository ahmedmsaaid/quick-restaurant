(function() {
    let token = localStorage.getItem('qs_admin_token') || localStorage.getItem('qs_vendor_token');
    let userJson = localStorage.getItem('qs_admin_user') || localStorage.getItem('qs_vendor_user');
    let isValid = false;
    ...
    if (token && userJson) {
        try {
            ...
            // Admin role must be 4 or string containing 'admin'
            if (r === 4 || (typeof r === 'string' && r.toLowerCase().includes('admin'))) {
                isValid = true;
            }
        } catch (_) {}
    }
            ...
            overlay.innerHTML = `
                ...
                ">
                    ...
                    <p style="font-size: 0.95rem; color: #5f6f72; line-height: 1.6; margin: 0 0 1.75rem 0;">
                        عفواً، لا توجد جلسة دخول صالحة للأدمن العام، أو انتهت صلاحية الجلسة الحالية. يرجى مسح البيانات المؤقتة وتسجيل الدخول بحساب أدمن معتمد.
                    </p>
