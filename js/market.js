...
async function updateUserSettings(days, hours, descriptionText) {
    const rawDescription = '__SETTINGS__:' + JSON.stringify({ days, hours, description: descriptionText });
    const profile = JSON.parse(localStorage.getItem('qs_vendor_user') || '{}');
    await apiFetch('/api/v1/users/update-profile', {
        method: 'PUT',
        body: JSON.stringify({ name: profile.name || 'Market', photo: profile.photo || '', description: rawDescription })
    });
...
function renderProfileTab(parent) {
    ...
    saveInfoBtn.addEventListener('click', async () => {
        ...
        const newRawDesc = '__SETTINGS__:' + JSON.stringify({ days: fresh.settings.days, hours: fresh.settings.hours, description: descIn.value.trim() });
        try {
            await apiFetch('/api/v1/users/update-profile', { method: 'PUT', body: JSON.stringify({ name, photo: currentPhoto, description: newRawDesc }) });
            const profile = JSON.parse(localStorage.getItem('qs_vendor_user') || '{}');
