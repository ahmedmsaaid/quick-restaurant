(function() {
    ...
    if (token && userJson) {
        try {
            ...
            // Restaurant role must be 0 or Admin 4
            if (r === 0 || r === 4 || (typeof r === 'string' && (r.toLowerCase().includes('rest') || r.toLowerCase().includes('admin')))) {
                isValid = true;
            }
        } catch (_) {}
    }
            ...
            overlay.innerHTML = `
                ...
                ">
                    ...
                    <p style="font-size: 0.95rem; color: #5f6f72; line-height: 1.6; margin: 0 0 1.75rem 0;">
                        عفواً، لا توجد جلسة دخول صالحة للمطعم، أو انتهت صلاحية الجلسة الحالية. يرجى مسح البيانات المؤقتة وتسجيل الدخول بحساب المطعم.
                    </p>
